...
//? if >=1.21.9
public class HairFeatureRenderer extends RenderLayer<AvatarRenderState, PlayerModel> {
//? if >=1.21.2 && <1.21.9
/*public class HairFeatureRenderer extends RenderLayer<PlayerRenderState, PlayerModel> {
*/
//? if <1.21.2
/*public class HairFeatureRenderer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
*/
...
//? if >=1.21.9 {
    public HairFeatureRenderer(RenderLayerParent<AvatarRenderState, PlayerModel> ctx) {
        super(ctx);
    }
//?}
//? if >=1.21.2 && <1.21.9 {
    public HairFeatureRenderer(RenderLayerParent<PlayerRenderState, PlayerModel> ctx) {
        super(ctx);
    }
//?}
//? if <1.21.2 {
    public HairFeatureRenderer(RenderLayerParent<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> ctx) {
        super(ctx);
    }
//?}
...
//? if >=1.21.11
        Identifier skinId = state.skin.body().texturePath();
//? if <1.21.11
        /*ResourceLocation skinId = state.skin.body().texturePath();
        */
        ...
        boolean slim = false;
        try {
            PlayerModelType skinType = state.skin.model();
        ...
        renderPlayerHair(player, matrices, mc.renderBuffers().bufferSource(), light, tickDelta, skinId);
    }
//?}
...
//? if >=1.21.2 && <1.21.9 {
    ...
    public void render(
        ...
        ResourceLocation skinId = state.skin == null ? null : state.skin.texture();
        if (skinId == null) return;

        boolean slim = false;
        try {
            slim = state.skin.model() == PlayerSkin.Model.SLIM;
        } catch (Exception ignored) {}
        SkinTextureCache.store(player.getUUID(), skinId, slim);

        float tickDelta = mc.getDeltaTracker().getGameTimeDeltaPartialTick(true);
        renderPlayerHair(player, matrices, consumers, light, tickDelta, skinId);
    }
//?}
...
//? if >=1.20.2 {
        ...
        if (playerSkin != null) {
            skinId = playerSkin.texture();
            slim = playerSkin.model() == PlayerSkin.Model.SLIM;
        }
//?}
//? if <1.20.2 {
        skinId = player.getSkinTextureLocation();
        try {
            slim = "slim".equalsIgnoreCase(player.getModelName());
        } catch (Exception ignored) {}
//?}
        if (skinId == null) return;
        SkinTextureCache.store(player.getUUID(), skinId, slim);

        renderPlayerHair(player, matrices, consumers, light, partialTick, skinId);
    }
//?}

//? if >=1.21.11
    private void renderPlayerHair(AbstractClientPlayer player, PoseStack matrices, MultiBufferSource consumers, int light, float tickDelta, Identifier skinId) {
//? if <1.21.11
    /*private void renderPlayerHair(AbstractClientPlayer player, PoseStack matrices, MultiBufferSource consumers, int light, float tickDelta, ResourceLocation skinId) {
    */
        List<StrandSimulation> sims = PhysicsTickHandler.MANAGER.get(player.getUUID());
        if (sims.isEmpty()) return;

//? if >=1.21.11
        RenderType renderLayer = RenderTypes.entityTranslucent(skinId, false);
//? if <1.21.11
        /*RenderType renderLayer = RenderType.entityTranslucent(skinId, false);
        */
...
//? if <1.21.11
    ...
    private static BoneFrame applyBoneTransform(PlayerModel model, StrandOrigin origin, PoseStack matrices) {
    ...
    private static float renderYaw(AbstractClientPlayer player, BoneFrame bone) {
        return bone.usesHeadPitch() ? player.getYHeadRot() : player.getVisualRotationYInDegrees();
    }
