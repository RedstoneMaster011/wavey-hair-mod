...
public class StrandSimulation {
    ...
    public void tick(double rootX, double rootY, double rootZ,
        ...
        if (cfg.windResponse > 0.0001f && player != null) {
            float phase = (player.tickCount + Math.floorMod(definition.id.hashCode(), 97)) * 0.12f;
    ...
    private void solveBodyCollision(AbstractClientPlayer player) {
        if (player == null) return;

        float yaw = (float) Math.toRadians(player.getVisualRotationYInDegrees());
...
//? if <1.20.2
    ...
    private float anchorYaw(AbstractClientPlayer player) {
        String bone = definition.origin.bone == null ? "" : definition.origin.bone.trim();
        return bone.equalsIgnoreCase("head") || bone.isBlank() ? player.getYHeadRot() : player.getVisualRotationYInDegrees();
    }
